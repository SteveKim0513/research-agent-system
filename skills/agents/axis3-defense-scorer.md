...
model: opus
purpose: 축 3 — 반박·강화 논리 (Steelman·Falsifiability·Limitations·Reviewer attack)
---
...
> **선로드 context 우선**: orchestrator가 prompt에 원고/critical-commitments를 주입한 경우 **Read 다시 X**. `papers/analyzed/{stage}/*` (stage = research-gap | flow) 는 직접 Read.

1. `flow/flow.md` 또는 `output/*.md`
2. `papers/analyzed/{stage}/*.md` 중 **axis_tags에 "steelman" 포함**한 것만 (3-5편 예상)
