...
model: opus
purpose: 축 6 — 비판 렌즈 (Paradigm·Fault-line·Bold Defense·Minority Recovery)
---
...
> **선로드 context 우선**: orchestrator가 prompt에 원고/critical-questions/critical-commitments를 주입한 경우 **Read 다시 X**. `papers/analyzed/{stage}/*` (stage = research-gap | flow) 는 직접 Read.

1. `flow/flow.md` 또는 `output/*.md`
2. `critical-questions.md` (사용자 답변)
3. `critical-commitments.md` (답변에서 추출된 actionable)
4. `papers/analyzed/{stage}/*.md` 중 **axis_tags에 "minority" 포함**한 것만 (3-5편 예상)
