...
⚠ **단방향**: ⑧ 초안 작성 후엔 flow(계획) 단계로 안 돌아갑니다. flow.md는 보존되지만 새로 수정 X.
...
A. 시스템은 단방향(flow → output)입니다. flow.md는 보존되니 *읽기 참조*는 가능하지만, *수정 후 재분석*은 권장 X. 정말 필요하면 `"flow에서 X를 보강하고 싶다"`라고 말해 AI 안내 받으세요.
